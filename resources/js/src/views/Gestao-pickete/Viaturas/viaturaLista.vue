    ...
    <b-card no-body class="mb-0">
        ...
        <b-table ref="refViaturaListTable" class="position-relative" :items="fetchViaturas" responsive :fields="tableColumns" primary-key="id" :sort-by.sync="sortBy" show-empty empty-text="No matching records found" :sort-desc.sync="isSortDirDesc">
            ...
            <template #cell(actions)="data">
                <b-dropdown variant="link" no-caret :right="$store.state.appConfig.isRTL" v-if="can('Update Viatura')">
                    ...
                    <b-dropdown-item v-if="data.item.estado == 0" @click="Activate(data.item.id)">
...
export default {
    ...
    setup() {
        ...
        function Activate(id) {
            store
                .dispatch("Picket/activateViatura", {
                    id
                })
                .then((response) => {
                    toast({
        ...
        function Deactivate(id) {
            store
                .dispatch("Picket/inactivateViatura", {
                    id
                })
                .then((response) => {
                    toast({
