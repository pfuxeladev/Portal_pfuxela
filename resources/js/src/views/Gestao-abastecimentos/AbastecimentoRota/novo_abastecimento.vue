  ...
  <section title="Novo-abastecimento" class="invoice-add-wrapper">
    <b-card no-body>
      <b-card-header class="pb-50 d-flex justify-content-center">
        <h3 class="text-center">Novo Abastecimento</h3>
      </b-card-header>
    </b-card>
    <b-row>
                  ...
                  <table class="table table-bordered table-stripped">
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td style="width: 20%">
                          <span class="text-primary">{{ TotalDistance }}</span>
                          Km's
                        </td>
...
export default {
  ...
  computed: {
    TotalDistance() {
      ...
      this.form.km_total = this.form.rota.reduce(
        (r, item) => r + item.distancia,
        0
      );
  ...
  methods: {
    getQtd() {
      //   alert(viatura_id)
      this.$http
        .get(`/api/getQtdDisponivel/${this.form.viatura_id}`)
        .then((res) => {
          this.rec_abast = res.data;
          console.log(this.rec_abast);
        });
      this.$http
        .get(`/api/getAlocateRoute/${this.form.viatura_id}`)
        .then((res) => {
    ...
    returnAbastecimento() {
      this.$http
        .get(`/api/CurrentAbst/${this.$route.params.refs}`)
        .then((res) => {
  ...
  setup() {
    ...
    function getSubmited() {
      this.$http
        .get(`/api/Ordems/${router.currentRoute.params.refs}`)
        .then((response) => {
    ...
    function enviarPedido(order) {
      this.$swal({
        ...
        if (result.value) {
          store
            .dispatch('Supply/addAbastecimento', order)
            .then((response) => {
              toast({
                component: ToastificationContent,
                props: {
              ...
              router.push({
                name: 'supply-details',
                params: { refs: router.currentRoute.params.refs },
              });
              //   window.location.reload()
            })
            .catch((err) => {
