    ...
    function ListarViaturas()
    {
        ...
        $viatura_rota = ViaturaRota::whereDate('created_at', Carbon::today())->get();
            ...
            if (auth()->user()->role('Gestor de Frotas') || auth()->user()->role('SuperAdmin') || auth()->user()->email === 'piquete@pfuxela.co.mz') {
                ...
                return Viatura::whereIn('id', $viatura)->where('viaturas.nome_viatura', '!=', 'BUS')->where('viaturas.locate', '=', 'IN')->where('viaturas.estado', true)->where('viaturas.updated_at', '>=', Carbon::now()->subDays(60))->get();
            }
    }
